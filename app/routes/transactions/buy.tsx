...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    await transactionService.buyStock({
    ...
    return json({
      errors: { submit: error.response?.data?.message || "Failed to buy stock" }
    }, { status: 400 });
  }
}
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Buy Stock</h1>
          <p className="text-gray-600">Add a new position to your portfolio</p>
        </div>
